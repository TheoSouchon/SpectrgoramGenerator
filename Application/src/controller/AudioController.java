...
public class AudioController implements Initializable {
	...
	void Open() {
		...
		// File("C:\\Users\\Michelle\\eclipse-workspace\\HEALT_APP\\python\\notesMusiques");
		...
		if (chosenFile != null) {
			...
			// anyCommand );
			// ProcessBuilder processBuilder = new ProcessBuilder("cmd.exe", "/c", "cd
			// python", "python Green.py");

			// ProcessBuilder builder = new
			// ProcessBuilder("python","C:\\Users\\Michelle\\eclipse-workspace\\HEALT_APP\\python\\Green.py",
			// path).inheritIO();
			...
			try {
				...
				/*
				 * p.waitFor(); BufferedReader bfr = new BufferedReader(new
				 * InputStreamReader(p.getInputStream())); String line = ""; while ((line =
				 * bfr.readLine()) != null) { System.out.println(line); if(p.waitFor()==1)
				 * System.out.println("elaboration is completed"); }
				 */
		...
		/*
		 * try { ProcessBuilder Process_Builder = new
		 * ProcessBuilder("python","HEALT_APP/src/Green.py") .inheritIO();
		 * 
		 * Process Demo_Process = Process_Builder.start(); Demo_Process.waitFor(); }
