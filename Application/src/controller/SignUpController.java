...
public class SignUpController implements Initializable {
	...
	// in the SignUpController.java file we have: iscription, btn_pass Buttons,
	...
	// this function is called when a user try to create an account by clicking the
	// button inscription
	@FXML
	void click() throws IOException {
		...
		try {
			...
			while (result.next()) {
				...
				// if the account is not already existing, then the account is created and the
				...
				if (result.isLast() && !nomeCheck && !mailCheck && !val) {
					try {
						...
						ps.setString(2, pass);
						ps.setString(3, mail);
						Alert alert = new Alert(AlertType.INFORMATION, " account create successfully",
					...
					Alert alert = new Alert(AlertType.ERROR, " Username and email already used",
							javafx.scene.control.ButtonType.OK);
					alert.showAndWait();
				}
				// if the provided username is already used, an alert is shown
					...
					Alert alert = new Alert(AlertType.ERROR, " Username  already used",
							javafx.scene.control.ButtonType.OK);
					alert.showAndWait();
				}
